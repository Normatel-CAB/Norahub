                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      {createExtras.filter(c => c.nome.trim()).length > 0 && (
                        <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-[#57B952]/20 text-[#57B952] text-[10px] font-bold">
                          {createExtras.filter(c => c.nome.trim()).length}
                        </span>
                      )}
                    </div>
                    <button type="button" onClick={addCard} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg bg-[#57B952]/10 text-[#57B952] border border-[#57B952]/20 hover:bg-[#57B952]/20 font-semibold transition-colors">
                      <Plus size={13} /> Novo card
                    </button>
                  </div>

                  {createExtras.length === 0 && (
                    <button type="button" onClick={addCard} className="w-full flex flex-col items-center justify-center gap-2 py-8 border border-dashed border-white/[0.10] rounded-2xl hover:border-[#57B952]/30 hover:bg-[#57B952]/[0.03] transition-all group">
                      ...
                      <p className="text-xs font-medium text-gray-500 group-hover:text-gray-400 transition-colors">Clique para adicionar um card</p>
                    </button>
                  )}
                    ...
                    <div className="space-y-3">
                      {createExtras.map((card, idx) => (
                        <div key={idx} className="group bg-white/[0.03] border border-white/[0.08] hover:border-white/[0.14] rounded-2xl p-4 space-y-3 transition-colors">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <span className="w-6 h-6 rounded-lg bg-[#57B952]/15 border border-[#57B952]/20 flex items-center justify-center text-[11px] font-bold text-[#57B952] flex-shrink-0">{idx + 1}</span>
                              <span className="text-xs text-gray-500 font-medium">Card {idx + 1}</span>
                            </div>
                            <button type="button" onClick={() => removeCard(idx)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-red-500/15 text-gray-600 hover:text-red-400 transition-all opacity-0 group-hover:opacity-100">
                              <X size={14} />
                            </button>
                          </div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-5 gap-2">
                            ...
                            <select value={card.tipo} onChange={e => updateCard(idx, 'tipo', e.target.value)} className={`sm:col-span-2 ${selectCls}`} style={SEL}>
                              {CARD_TYPES.map(t => <option key={t.value} value={t.value} style={OPT}>{t.label}</option>)}
                            </select>
                          </div>

                          <input type="text" placeholder="Descrição (opcional)" value={card.descricao} onChange={e => updateCard(idx, 'descricao', e.target.value)} className={inputCls} />

                          {!NO_URL_TYPES.includes(card.tipo) && (
                            <input type="text" placeholder="URL (https://...)" value={card.url} onChange={e => updateCard(idx, 'url', e.target.value)} className={inputCls} />
                          )}
                          {NO_URL_TYPES.includes(card.tipo) && (
                            <p className="flex items-center gap-2 text-xs text-blue-300 bg-blue-500/10 border border-blue-400/20 rounded-xl px-3 py-2"><span>📁</span> Permite upload de arquivos após criado</p>
                          )}
                        </div>
                      ))}
                    </div>
